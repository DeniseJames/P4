...
# The following code to retrieve the mtx, dist camera coefficients tested
# in camera_calibration notebook.
# 
...
def abs_sobel_thresh(img, orient='x', sobel_kernel=5, thresh=(30, 150)):
...
ksize = 15
sobelx_thresh = (30, 150)
sobely_thresh = (30, 150)
mag_grad_thresh = (20, 250)
dir_grad_thresh = (0.3, 1.3)
#dir_grad_thresh=(0, np.pi/2).

# Apply each of the thresholding functions
gradx = abs_sobel_thresh(birdseye, orient='x', sobel_kernel=ksize, thresh=sobelx_thresh)
grady = abs_sobel_thresh(birdseye, orient='y', sobel_kernel=ksize, thresh=sobely_thresh)
...
mag_binary = mag_threshold(birdseye, sobel_kernel=9, mag_thresh = mag_grad_thresh)
...
dir_binary = dir_threshold(birdseye, sobel_kernel= 15, thresh = dir_grad_thresh)
# plot the result
...
ax1.set_title('birdseye', fontsize = 30)
ax2.imshow(s_channel, cmap = 'gray')
ax2.set_title('v_channel', fontsize=40)


# In[21]:

def hls_select_i(img, thresh=(10, 20)):
    hls = cv2.cvtColor(img, cv2.COLOR_RGB2HLS)
    l_channel = hls[:,:,1]
...
l_channel = hls_select_i(birdseye)
...
# 
# 
# hsv = cv2.cvtColor(birdseye, cv2.COLOR_RGB2HSV)
# 
# mask = np.zeros_like(hsv[:,:,2])
# mask[(hsv[:,:,2]>0)& (hsv[:,:,2]< 255)]=1
# 
# v_channel = hsv_select_v_channel(birdseye)
# mag_binary = mag_threshold(birdseye, sobel_kernel=ksize, mag_thresh = mag_grad_thresh)
# final_mask = np.maximum(v_channel, mag_binary)
...
histogram = np.sum(s_channel[s_channel.shape[0]//2:,:], axis=0)
plt.plot(histogram)
...
def process_image(image):
   ...
   # define 4 source points for perspective transformation
    ...
    binary_warped  = hls_select(birdseye) 
